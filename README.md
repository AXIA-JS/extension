...
- On Chrome, install via [Chrome web store](https://chrome.google.com/webstore/detail/axia%7Bjs%7D-extension/mopnmbcafieddcagagdcbnhejhlodfdd)
- On Firefox, install via [Firefox add-ons](https://addons.mozilla.org/en-US/firefox/addon/axia-js-extension/)
...
3. When visiting `https://axia.js.org/apps/` it will inject the extension

Once added, you can create an account (via a generated seed) or import via an existing seed. The [apps UI](https://github.com/axia-js/apps/), when loaded, will show these accounts as `<account name> (extension)`
...
The documentation for Dapp development is available [in the axia-js doc](https://axia.js.org/docs/extension).

This approach is used to support multiple external signers in for instance [apps](https://github.com/axia-js/apps/). You can read more about the convenience wrapper [@axia-js/extension-dapp](packages/extension-dapp/) along with usage samples.
...
From a dapp developer perspective, the only work needed is to include the [@axia-js/extension-dapp](packages/extension-dapp/) package and call the appropriate enabling function to retrieve all the extensions and their associated interfaces.

From an extension developer perspective, the only work required is to enable the extension via the razor-thin [@axia-js/extension-inject](packages/extension-inject/) wrapper. Any dapp using the above interfaces will have access to the extension via this interface.
...
interface Injected {
  ...
  // not injected as of yet, subscribable provider for axia-js API injection,
...
interface Signer extends SignerInterface {
  ...
  // the axia-js API, confirming the Signer interface for this API
}
```

## Injection information

The information contained in this section may change and evolve. It is therefore recommended that all access is done via the [@axia-js/extension-dapp](packages/extension-dapp/) (for dapps) and [extension-inject](packages/extension-inject/) (for extensions) packages, which removes the need to work with the lower-level targets.
...
window.injectedWeb3 = {
  ...
  // each with their own keys, here `axia-js` is for this extension
  'axia-js': {
...
When you create a keypair via the extension, it supplies a 12-word mnemonic seed and asks you to create a password. This password only encrypts the private key on disk so that the password is required to spend funds in `axia-js/apps` or to import the account from backup. The password does not protect the mnemonic phrase. That is, if an attacker were to acquire the mnemonic phrase, they would be able to use it to spend funds without the password.
