...
import type { AuthorizeRequest } from '@axia-js/extension-base/background/types';
...
describe('Authorize', () => {
  ...
  it('render requests', () => {
    const wrapper = mountAuthorize([{ id: '1', request: { origin: '???' }, url: 'http://axia-js.org' }]);
  ...
  it('render more request but just one accept button', () => {
    const wrapper = mountAuthorize([
      { id: '1', request: { origin: '???' }, url: 'http://axia-js.org' },
      { id: '2', request: { origin: 'abc' }, url: 'http://axia-js.pl' }
    ]);
