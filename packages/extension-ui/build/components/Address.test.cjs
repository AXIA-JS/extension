...
var _metadataMock = require("../Popup/Signing/metadataMock.cjs");
...
const accounts = [{
  ...
  name: 'Ethereum',
  type: 'ethereum'
}, _objectSpread({}, externalAccount), _objectSpread({}, hardwareAccount)]; // With AlphaNet genesis Hash
// This account isn't part of the generic test because AlphaNet isn't a built in network
// The network would only be displayed if the corresponding metadata are known

const alphaNetAccount = {
...
}, // with AXIALunar genesis Hash
{
  ...
  expectedIconTheme: 'axia',
  expectedNetworkLabel: 'AXIALunar',
  genesisHash: '0xb0a8d493285c2df73290dfb7e61f870f17b41801197a149ca93654499ea3dafe',
  type: 'sr25519'
...
const mountComponent = async (addressComponentProps, contextAccounts) => {
  const actionStub = jest.fn();
  const {
    actions = actionStub
  } = addressComponentProps;
  const wrapper = (0, _enzyme.mount)( /*#__PURE__*/(0, _jsxRuntime.jsx)(_index.AccountContext.Provider, {
    value: {
      accounts: contextAccounts,
      hierarchy: (0, _buildHierarchy.buildHierarchy)(contextAccounts)
    },
    children: /*#__PURE__*/(0, _jsxRuntime.jsx)(_index.Address, _objectSpread({
      actions: actions
    }, addressComponentProps))
  }));
...
const getWrapper = async (account, contextAccounts, withAccountsInContext) => {
  ...
  const mountedComponent = withAccountsInContext // only the address is passed as props, the full acount info are loaded in the context
  ? await mountComponent({
    address: account.address
  }, contextAccounts) // the context is empty, all account's info are passed as props to the Address component
  : await mountComponent(account, []);
  return mountedComponent.wrapper;
};

const genericTestSuite = (account, withAccountsInContext = true) => {
  let wrapper;
  const {
    address,
    expectedIconTheme,
  ...
  describe(`Account ${withAccountsInContext ? 'in context from address' : 'from props'} (${name}) - ${type}`, () => {
    ...
    it('can show the account if hidden', async () => {
      ...
      const mountedHiddenComponent = withAccountsInContext ? await mountComponent(_objectSpread({
        address
      }, additionalProps), accounts) : await mountComponent(_objectSpread(_objectSpread({}, account), additionalProps), []);
    ...
    it('has no account hidding and settings button if no action is provided', async () => {
      ...
      const mountedComponentWithoutAction = withAccountsInContext ? await mountComponent(_objectSpread({
        address
      }, additionalProps), accounts) : await mountComponent(_objectSpread(_objectSpread({}, account), additionalProps), []);
...
const genesisHashTestSuite = (account, withAccountsInContext = true) => {
  const {
...
describe('Address', () => {
  ...
  describe('Encoding and label based on Metadata', () => {
    ...
    beforeAll(async () => {
      ...
      wrapper = await getWrapper(alphaNetAccount, [], false);
    });
    it('shows alphanet label with the correct color', () => {
      var _bannerChain$prop;

      const bannerChain = wrapper.find('[data-field="chain"]');
      expect(bannerChain.text()).toEqual(_metadataMock.alphanetMetadata.chain);
      expect((_bannerChain$prop = bannerChain.prop('style')) === null || _bannerChain$prop === void 0 ? void 0 : _bannerChain$prop.backgroundColor).toEqual(_metadataMock.alphanetMetadata.color);
    });
    it('shows the account correctly reencoded', () => {
      expect(wrapper.find('[data-field="address"]').text()).toEqual(alphaNetAccount.expectedEncodedAddress);
    });
  });
  describe('Derived accounts', () => {
    ...
    it('shows the parent account and suri', () => {
      const expectedParentNameSuri = (0, _getParentNameSuri.default)(childAccount.parentName, childAccount.suri);
