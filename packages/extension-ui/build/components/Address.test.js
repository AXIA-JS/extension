...
import { alphanetMetadata } from "../Popup/Signing/metadataMock.js";
...
import { AccountContext, Address } from "./index.js"; // eslint-disable-next-line @typescript-eslint/no-unsafe-assignment,@typescript-eslint/no-unsafe-call
...
const mountComponent = async (addressComponentProps, contextAccounts) => {
  ...
  const wrapper = mount( /*#__PURE__*/_jsx(AccountContext.Provider, {
    value: {
      accounts: contextAccounts,
      hierarchy: buildHierarchy(contextAccounts)
    },
    children: /*#__PURE__*/_jsx(Address, _objectSpread({
      actions: actions
    }, addressComponentProps))
  }));
...
describe('Address', () => {
  ...
  describe('Encoding and label based on Metadata', () => {
    let wrapper;
    beforeAll(async () => {
      jest.spyOn(MetadataCache, 'getSavedMeta').mockResolvedValue(alphanetMetadata);
    ...
    it('shows alphanet label with the correct color', () => {
      ...
      expect(bannerChain.text()).toEqual(alphanetMetadata.chain);
      expect((_bannerChain$prop = bannerChain.prop('style')) === null || _bannerChain$prop === void 0 ? void 0 : _bannerChain$prop.backgroundColor).toEqual(alphanetMetadata.color);
    });
